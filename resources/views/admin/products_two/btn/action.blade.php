<div class="btn-group">
    ...
    <ul class="dropdown-menu" role="menu">
        <li><a href="{{url(route('admin.products_two.edit', $id))}}">{{trans('datatable.edit')}}</a></li>
        <li><a href="#" onclick="deleteConfirmationPlace({{$id}})">{{trans('datatable.delete')}}</a></li>
    </ul>
</div>
...
<script type="text/javascript">
    function deleteConfirmationPlace(id) {
        swal({
            ...
            if (e.value === true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{url('admin/products_two/delete')}}/" + id,
                    data: {_token: CSRF_TOKEN},
                    dataType: 'JSON',
                    success: function (results) {

                        if (results.success === true) {
                            $('#datatable_place').DataTable().ajax.reload();
